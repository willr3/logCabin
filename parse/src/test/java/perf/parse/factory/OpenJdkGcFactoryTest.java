...
public class OpenJdkGcFactoryTest {
    ...
    @Test public void heap(){
        ...
        System.out.println(p.getBuilder().getRoot().toString(2));
    ...
    @Test public void region(){
        Parser p = new Parser();
        p.add(f.newRegionPattern().debug());
        ...
        System.out.println(p.getBuilder().getRoot().toString(3));
    }
    ...
    public void combined(){
        ...
        p.add(new Exp("tenured", "PERF tenuring (?<class>\\S+) ").group("tenured").set("class", Exp.Value.Count));
